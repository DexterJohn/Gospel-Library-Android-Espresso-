...
class SearchPreviewContentAdapter : RecyclerHeaderAdapter<StandardListHeaderViewHolder, SearchPreviewContentAdapter.ViewHolder>() {
    var itemClickListener: (SearchPreviewSubitem) -> Unit = {}

    var list: List<SearchPreviewSubitem> = listOf()
        set(value) {
            field = value
    ...
    override fun onBindChildViewHolder(holder: ViewHolder, childPosition: Int) {
        ...
        holder.titleTextView.text = searchPreviewItem.title
        holder.previewTextView.setHtml(searchPreviewItem.text)

        if (searchPreviewItem.count > 1) {
            holder.resultsTextView.setVisible(true)
            holder.resultsTextView.text = holder.itemView.context.resources.getQuantityString(R.plurals.search_count_results, searchPreviewItem.count.toInt(), searchPreviewItem.count)
        } else {
